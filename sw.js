...
self.addEventListener('fetch', e => {
  ...
  if (url.hostname.includes('tile.openstreetmap') || url.hostname.includes('basemaps')) {
    e.respondWith(caches.open('garmin-tiles').then(async c => { const r = await c.match(e.request); if (r) return r; try { const n = await fetch(e.request); if (n.ok) c.put(e.request, n.clone()); return n; } catch (x) { return r || Response.error(); } }));
    return;
  }
  e.respondWith((async () => {
