...
async function renderIntegracoes() {
  ...
  $('#iStr').onclick = () => { if (j.itens.strava_plataforma || st?.client_id) { location.href = 'strava.php'; return; } modal(`<h2>⌚ Strava (ponte do Haylou)</h2><p class="mini">1) Entre em <a href="https://www.strava.com/settings/api" target="_blank">strava.com/settings/api</a> e crie um aplicativo (nome livre, site <b>alequizao.com</b>, <b>Authorization Callback Domain: alequizao.com</b>).<br>2) Copie o Client ID e o Client Secret abaixo.<br>3) Toque em Salvar e autorize no Strava.</p><form id="fStr"><div class="campo"><label>Client ID</label><input name="client_id" required inputmode="numeric"></div><div class="campo"><label>Client Secret</label><input name="client_secret" required></div><button class="btn">Salvar e autorizar</button></form>`, () => { $('#fStr').onsubmit = async e => { e.preventDefault(); const f = Object.fromEntries(new FormData(e.target)); await api('integracao_salvar', { servico: 'strava', ...f }); location.href = 'strava.php'; }; }); };
...
async function renderWalkie() {
  ...
  ${widget({ ico: '🔔', cor: '#3cc7a8', titulo: 'Receber nas notificações do relógio', corpo: `<ol class="passos"><li>Neste celular, toque em <b>Ativar notificações</b> (no iPhone, primeiro adicione o site à Tela de Início).</li><li>No app <b>Garmin Connect</b>: Mais → Configurações → Notificações → deixe permitido o navegador (Chrome/Safari).</li><li>Pronto: mensagens, "chamar atenção" e SOS do canal chegam no celular e aparecem no relógio, mesmo com o app fechado.</li></ol><div class="linha"><button class="btn peq" id="wkPush2">🔔 Ativar notificações</button><button class="btn sec peq" id="wkPushTeste">Enviar teste</button></div>` })}` : ''}`;
