...
if ($lat !== null && $posNova) {
  $q += ['lat' => $lat, 'lon' => $lon, 'altitude' => $num('alt'), 'speed' => $num('vel') !== null ? round($num('vel') * 1.943844, 2) : null, 'bearing' => $num('rumo'),
         'timestamp' => $fixTs, 'valid' => 'true', 'gpsIdade' => $agora - $fixTs];
} else {
  // sem GPS novo (o relógio não liga o GPS em segundo plano): manda só bateria/saúde, sem lat/lon.
  // O Traccar mantém o último ponto REAL (relógio, LiveTrack ou atividade) com o horário original dele,
  // em vez de repetir um ponto velho com horário novo.
  $q += ['timestamp' => $agora, 'posicao' => 'sem_gps_novo', 'gpsIdade' => $fixTs ? $agora - $fixTs : null];
}
...
// comando pendente do painel web (ex.: iniciar caminhada no relógio) — entregue uma única vez e apagado
$resp = ['ok' => true, 'traccar' => $traccar];
if (($d['origem'] ?? '') === 'ao_vivo') { // só entrega ao app ABERTO (foreground), que consegue gravar de forma confiável
  $cst = db()->prepare("SELECT json FROM garmin_dados WHERE usuario_id=? AND tipo='cmd_relogio' AND chave='pendente' LIMIT 1");
  $cst->execute([RELOGIO_UID]);
  if ($cj = $cst->fetchColumn()) {
    $c = json_decode($cj, true);
    if ($c && (time() - (int)($c['ts'] ?? 0) <= 300)) { $resp['comando'] = $c['cmd']; $resp['sport'] = $c['sport'] ?? 'caminhada'; } // ignora comando velho (>5 min)
    db()->prepare("DELETE FROM garmin_dados WHERE usuario_id=? AND tipo='cmd_relogio' AND chave='pendente'")->execute([RELOGIO_UID]);
  }
}
