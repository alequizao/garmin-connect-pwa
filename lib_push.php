 ...
 */
// chaves VAPID guardadas na tabela config_app deste sistema
if (!function_exists('cfg_get')) {
  function cfg_get(string $k, $padrao = null) { $st = db()->prepare("SELECT valor FROM config_app WHERE chave=?"); $st->execute([$k]); $v = $st->fetchColumn(); return $v === false ? $padrao : $v; }
  function cfg_set(string $k, $v): void { db()->prepare("INSERT INTO config_app (chave, valor) VALUES (?,?) ON DUPLICATE KEY UPDATE valor=VALUES(valor)")->execute([$k, $v]); }
}
...
{
    $pub  = (string) cfg_get('garmin_vapid_pub', '');
    $priv = (string) cfg_get('garmin_vapid_priv', '');
    if ($pub === '' || $priv === '') {
        ...
        $priv = (string) $privPem;
        cfg_set('garmin_vapid_pub', $pub);
        cfg_set('garmin_vapid_priv', $priv);
    }
    $sub = (string) cfg_get('garmin_vapid_sub', '');
    if ($sub === '') {
        $sub = 'mailto:' . (defined('DEV_EMAIL') ? DEV_EMAIL : (defined('SMTP_DE') && SMTP_DE ? SMTP_DE : 'contato@example.com'));
    }
